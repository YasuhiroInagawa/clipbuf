...
- `clipboard/` — クリップボードの監視・読み書き。`mod.rs` に共通トレイト `ClipboardPort`（監視開始、読み取り、書き込み、取り込み能力）を定義し、`clipboard_rs.rs` が `clipboard-rs` で 3 OS を実装する。`marker.rs`（自己書き込み判定）、`conceal.rs`（秘匿マーク）、`fake.rs`（テスト用）もここ。OS 固有の補助（macOS の許可状態確認など）もこのモジュール内に閉じる

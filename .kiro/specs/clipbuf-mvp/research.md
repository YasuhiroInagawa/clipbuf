...
### Decision: グローバルホットキーの既定値は `Alt+Shift+KeyV`
- **Rationale**: `Ctrl/Cmd+Shift+V` は多くのアプリの「プレーンテキストで貼り付け」と衝突する。`Alt+Shift+V` は主要 OS・主要アプリの既定と衝突しにくい。設定で変更可能。表記は記録 UI が出す物理キー名（`KeyV`）に揃える

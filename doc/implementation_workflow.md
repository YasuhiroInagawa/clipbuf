...
### 5.4b フィードバック反映（実画面を見て出た指摘）
- ホバーでしか出ないボタンは「押せるもの」と認識されにくく、`:focus-within` でクリック後に残る不具合も招いた。**常時表示 + テキストラベル + 枠付き**にして、状態表示（警告アイコン）と操作（ボタン）を見た目で分けた
- 「原文」ボタンは書式付きの項目にだけ出す。書式がない項目では行クリックと結果が同じで、ボタンの存在が疑問を生むため（要件 6.4.1 を追加）
- 全文プレビューは **Rust の `preview_transfer`** が返す。転送と同じ `resolve_transfer` を共有するので「見た内容＝貼られる内容」。フロントで変換を再実装しない
- `mouseenter` は**バブルしない**ため、子要素（プレビュー行）へのホバーで親のハンドラは呼ばれない。テストも実装も `mouseover` に統一した
- testing-library の mount オプションと同名の props（`anchor`）は渡せない。`anchorEl` に改名
- Svelte 5 の `$derived` 内で `$state` を書き換えるとレンダリングが止まる。派生値は 1 つのオブジェクトにまとめて返す
- 可視化した記号には `aria-label` だけでなく **`title` も付ける**（スクリーンリーダー用の属性はマウスでは見えない）。指摘されるまで気づかなかった
- Vitest はデフォルトで CSS を処理しないため `toBeVisible()` が常に真になる。`test.css: true` を有効にして「常時表示」を実際に検証

### 5.5 設定ウィンドウ（実画面のフィードバックを多く反映）
- **jsdom で通っても実機で動かない代表例**：macOS の WebKit は**ボタンをクリックしてもキーボードフォーカスが移らない**。ボタンに付けた `keydown` は呼ばれず、ホットキー記録が無反応だった。記録中は `<svelte:window onkeydown>` で拾う形に変更
- グローバルホットキーは OS に登録されているので、記録中は **Rust 側で一時解除**しないと既定キーを押した瞬間にメインウィンドウが反応してしまう（`suspend_hotkey` / `resume_hotkey`）
- 範囲検証をフィールド単位で出すため、Rust の範囲定数を **契約 fixture 経由でフロントと共有**（`settingsRanges`）。バックエンドの `validate` と同じ境界であることを Rust のテストで固定
- Vitest のテスト間で **i18n シングルトンの言語が漏れる**。言語を変えるテストがあるなら `beforeEach` でリセットする（`beforeAll` では足りない）
- ポップオーバーを操作可能にする場合、(a) `pointer-events: auto`、(b) 行とポップオーバーの隙間を跨げる猶予、(c) **リサイズのドラッグ中は閉じない**（mousedown 〜 mouseup の間は leave を無視）の 3 つが要る
- CSS の `resize` は `max-width` / `max-height` に縛られる。初期サイズは JS で内容から決め、`max-*` は上限（ビューポート）に使う
- Web UI なので**ポップオーバーはメインウィンドウの外に出られない**。「画面いっぱい」にはできない（ユーザーと確認のうえ現状維持）

### ホバー UI の着地点（5.5 後のフィードバック）
- ポップオーバーを「掴める」（スクロール・リサイズ可）ものにすると、「触れたら消える」軽さとは両立しない。今回は掴める側を採り、代わりに**開くまで 500ms 待つ**ことで「目的の行へ移動する途中で開いて邪魔をする」問題を解消した
- 配置は行の**左下**。左上に合わせると行を覆ってクリック転送ができなくなる（実装前に指摘して回避）
...
3. 確認項目
   ...
   - 「すべて削除」と空表示
4. 5.4 / 5.4b / 5.5 は完了済み。次は 5.6（更新の通知と承諾）→ 6.x（統合・リリース）
